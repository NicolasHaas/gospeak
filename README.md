...
> **Note:** Voice packets and screen-share frames are encrypted with AES-128-GCM keys generated by the server and distributed to authorized clients over TLS. The server relays both without needing to decode them, but a compromised or modified server _could_ still decrypt them because it generates the keys. See the [security threat model](docs/security.md) for details.
